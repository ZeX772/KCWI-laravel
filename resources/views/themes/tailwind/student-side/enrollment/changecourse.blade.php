...
@section('content')
            ...
            <div class="d-flex flex-column" id="content-wrapper"
                ...
                <p class="d-xl-flex justify-content-xl-center align-items-xl-center p24bb"
                    style="margin-bottom: 0px;height: 31px;"><strong>Change Course</strong></p>
                <div class="d-xl-flex d-xxl-flex justify-content-xl-center justify-content-xxl-center">
                    <div style="width: auto;" id="student-list_container">
                        <p class="p18b" style="margin-top: 50px;margin-bottom: 0px;"><strong>Choose Student and Change
                                Course</strong></p>
                        
                            @csrf
                            @if (!empty($students))
                                @foreach($students as $student)
                                    <form id="radioForm" class="form_{{$loop->iteration}}" action="{{ route('wave.storeChangeCourseStudentInfo') }}" method="post">
                                        <div style="margin-bottom: 20px;" class="student-card_container" data-index="{{$loop->iteration}}">
                                            <label for="student_{{$loop->iteration}}" class="d-xl-flex d-xxl-flex justify-content-around align-items-xl-center align-items-xxl-center divcard student-card cursor-pointer"
                                                ...
                                                <input type="radio" id="student_{{$loop->iteration}}" name="student_name" value="{{$student['name']}}" style="display: none;">
                                                <input type="hidden" name="student_id" value="{{$student['id']}}">
                                                <div class="avatar-circle_container"><img src="{{ $student['image_path'] }}" style="width: 101px;"></div>
                                                <div>
...
<script>
    $(document).ready(function() {
        ...
        {
            $('#student-list_container').on('click', '.student-card_container', function(){
