            ...
            <div class="d-flex flex-column" id="content-wrapper"
                ...
                <div class="d-xl-flex d-xxl-flex justify-content-xl-center justify-content-xxl-center">
                    <div style="width: auto;">
                        <p class="p18b" style="margin-top: 50px;margin-bottom: 0px;">Choose student and get started
                            the</br>enrollment</p>
                            <form id="radioForm" action="{{ route('wave.storeFlexibleStudentInfo') }}" method="post">
                                @csrf
                                @if (!empty($students))
                                    @foreach($students as $student)
                                        <div style="margin-bottom: 20px;">
                                            <label for="student_{{$loop->iteration}}" class="d-xl-flex d-xxl-flex justify-content-around align-items-xl-center align-items-xxl-center divcard student-card"
                                                style="padding-right: 20px;padding-left: 20px;padding-top: 20px;padding-bottom: 20px; text-decoration: none; color: black;">
                                                <input type="radio" id="student_{{$loop->iteration}}" name="studentInfoFlexible" value="{{$student['name']}}" style="display: none;">
                                                ...
                                                <div class="d-xl-flex d-xxl-flex align-items-xl-center align-items-xxl-center">
                                                    <p class="p18b" style="border: 1px solid; border-radius: 15px; padding: 0 20px; margin: 0;">
                                                        <strong><input type="hidden" name="selectedStudentInfo" value="{{$student['id']}}">
                                                            {{optional($student['student_information'])['hkid']}}</strong>
                                                    </p>
