...
@section('content')
            ...
            <div class="d-flex flex-column" id="content-wrapper"
                ...
                <p class="d-xl-flex justify-content-xl-center align-items-xl-center p24bb"
                    style="margin-bottom: 0px;height: 31px;"><strong>Full Term Enrollment</strong></p>
                ...
                <div class="d-xl-flex d-xxl-flex justify-content-xl-center justify-content-xxl-center">
                    <div style="width: auto;">
                        <p class="p18b" style="margin-top: 50px;margin-bottom: 0px;">Choose student and get started the</br>enrollment</p>
                        <form id="radioForm" action="{{ route('wave.storeFullTermStudentInfo') }}" method="post">
                            @csrf
                            @if (!empty($students))
                                @foreach($students as $student)
                                    <div style="margin-bottom: 20px;">
                                        <label for="student_{{$loop->iteration}}" class="d-xl-flex d-xxl-flex justify-content-around align-items-xl-center align-items-xxl-center divcard student-card cursor-pointer"
                                            ...
                                            <div style="width: 215px;">
                                                <p class="p22bold" style="margin-bottom: 15px;"><input type="radio" id="student_{{$loop->iteration}}" name="studentInfoFullTerm" value="{{ $student['name'] }}" style="display: none;">{{$student['name']}}</p>
                                                <p class="ripple-beginner" style="margin-top: 15px;">Ripple Beginner 2</p>
                                            </div>
                                            <div class="d-xl-flex d-xxl-flex align-items-xl-center align-items-xxl-center">
                                                <p class="p18b" style="border: 1px solid; border-radius: 15px; padding: 0 20px; margin: 0;">
                                                    <strong><input type="hidden" name="selectedStudentInfo" value="{{$student['id']}}">{{optional($student['student_information'])['hkid']}}</strong>
                                                </p>
...
@section('javascript')
<script>
    // Add event listeners to student cards to trigger form submission
    document.querySelectorAll('.student-card').forEach(item => {
        item.addEventListener('click', event => {
            // Find the radio button within the clicked student card
            const radioButton = item.querySelector('input[type="radio"]');
            // Check if the radio button exists and is not checked
