    ...
    <div class="d-xl-flex d-xxl-flex justify-content-xl-center justify-content-xxl-center">
        <div style="width: 50%; padding-top: 30px;">
            <div class="progress beautiful progress-xs" style="height: 8px;border-top-left-radius: 50px;border-top-right-radius: 50px;border-bottom-right-radius: 50px;border-bottom-left-radius: 50px;">
                <div class="progress-bar" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100" style="width: 25%; background: var(--app-color-tone-testing, linear-gradient(180deg,rgba(127, 158, 211, 1) 0%, rgba(165, 208, 245, 1) 100%));"><span class="visually-hidden">25%</span></div>
            </div>
            <div style="margin-top: 50px;">
                <p class="p24b" style="text-align: center;">Level</p>
                <p class="p16b" style="text-align: center;">Please select the level (1/4)</p>
            </div>
            <form id="radioForm" action="{{route('wave.storeFullTermEnrollLevel')}}" method="post" onsubmit="return validate();">
                @csrf
                ...
                @foreach($levels['response']['levels'] as $level)
                    <div class="d-xl-flex d-xxl-flex align-items-xl-center align-items-xxl-center" style="margin-bottom: 10px;">
                        <input type="radio" name="levelFullTerm" class="levelOption" id="option1" value="{{ $level['id'] }}" data-abbreviation="{{ $level['abbreviation'] }}">
            ...
            @endif
                <div class="d-xl-flex d-xxl-flex justify-content-xl-end justify-content-xxl-end align-items-xxl-center" style="margin-top: 50px;"><button class="button1 next-button" type="submit">Next<svg xmlns="http://www.w3.org/2000/svg" viewBox="-96 0 512 512" width="1em" height="1em" fill="currentColor" style="margin-left: 20px;">
                        ...
                        <input type="hidden" id="selectedAbbreviation" name="selectedAbbreviation" value="">
            </form>
        </div>
    </div>
...
<script>
    function validate() {
        var radioButtons = document.getElementsByName('levelFullTerm');
        var isChecked = false;
    ...
    $(document).ready(function() {
        $('input.levelOption').change(function() {
            // Get the selected abbreviation from the data-abbreviation attribute of the selected radio button
            var abbreviation = $(this).data('abbreviation');
            
            // Set the value of the hidden input field to the selected abbreviation
            $('#selectedAbbreviation').val(abbreviation);
        });
    });
</script>
@endsection
