...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function studentRegister(Request $request){
        $apiUrl2 = config('services.api_url') . '/register';
        ...
        $apiResponse = post($apiUrl2, $requestData);
    
            $token = $apiResponse->json('token');
            session(['registered_user_email' => $request->email]);

        
            return redirect()->route('verify-otp')->with('token', $token);
    }
