...
class SubscriptionController extends Controller
{
    ...
    public function checkout(Request $request){
        ...
        //PaddleSubscriptions
        $response = get($this->paddle_checkout_url . '/1.0/order?checkout_id=' . $request->checkout_id);
        $status = 0;
        $message = '';
        ...
        if( $response->successful() ){
            ...
            if(isset($resBody->order)){
                ...
                if($order->is_subscription && $plans->contains('plan_id', $order->product_id) ){
                    ...
                    $subscriptionUser = post($this->paddle_vendors_url . '/2.0/subscription/users', [
    ...
    public function invoices(User $user){
        ...
        if(isset($user->subscription->subscription_id)){
            $response = post($this->paddle_vendors_url . '/2.0/subscription/payments', [
                'vendor_id' => $this->vendor_id,
                'vendor_auth_code' => $this->vendor_auth_code,
    ...
    public function switchPlans(Request $request){
        ...
        if(isset($plan->id)){
            ...
            $response = post($this->paddle_vendors_url . '/2.0/subscription/users/update', [
                'vendor_id' => $this->vendor_id,
                'vendor_auth_code' => $this->vendor_auth_code,
